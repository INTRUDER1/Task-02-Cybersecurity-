...
def decrypt_image(encrypted_image_path):
    ...
    decrypted_path = encrypted_image_path.rsplit('_encrypted', 1)[0] + "_decrypted.png"
...
def main():
    while True:
        ...
        if choice == 'e':
            image_path = input("Enter the path of the image to encrypt: ").strip().strip('"')
            encrypt_image(image_path)
        elif choice == 'd':
            encrypted_image_path = input("Enter the path of the encrypted image to decrypt: ").strip().strip('"')
